 ...
 * 不做通配符、不识别管道 {@code |} / 分号 {@code ;} / 反引号。仅支持单/双引号包裹带空格的参数，
 * 以及把重定向符 {@code >} / {@code >>} 作为独立 token（供 echo 写文件用）。最终参数以字符串数组
 * 形式交给执行层，绝不拼接成 shell 字符串。
 */
@Component
public class CommandParser {
     ...
     * 未加引号的 {@code >} / {@code >>} 单独成词。不识别其它 shell 元字符。
     */
